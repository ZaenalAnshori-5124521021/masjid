import { Calendar, Trash2, RotateCcw, PlusCircle, TrendingDown } from 'lucide-react';
...
type ExpensePageProps = {
  ...
  setExpenseData: (data: Transaction[]) => void;
};

export default function ExpensePage({ expenseData, setExpenseData }: ExpensePageProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newExpense = {
      id: expenseData.length > 0 ? Math.max(...expenseData.map(i => i.id)) + 1 : 1,
  ...
  const totalAmount = expenseData.reduce((sum, item) => sum + item.numericAmount, 0);
    ...
    <div className="flex-1 p-3 sm:p-4 lg:p-8 overflow-auto bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="mb-4 sm:mb-6 lg:mb-8">
          <p className="text-[10px] sm:text-xs lg:text-sm text-gray-500 mb-2">Dashboard / Pengeluaran</p>
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-1">Manajemen Pengeluaran</h1>
          <p className="text-xs sm:text-sm lg:text-base text-gray-600">Kelola data pengeluaran keuangan masjid</p>
        </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 shadow-md border border-gray-200/50">
              <div className="flex items-center gap-2 lg:gap-3 mb-3 sm:mb-4 lg:mb-6">
                <div className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-emerald-100 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
                  <PlusCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-emerald-600" />
                </div>
                <h2 className="text-base sm:text-lg lg:text-xl font-bold text-gray-900">Tambah Pengeluaran</h2>
              </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 shadow-md border border-gray-200/50">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-3 sm:mb-4 lg:mb-6 gap-3 sm:gap-4">
                <div className="flex items-center gap-2 lg:gap-3">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-emerald-100 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
                    <TrendingDown className="w-3.5 h-3.5 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-emerald-600" />
                  </div>
                  <h2 className="text-base sm:text-lg lg:text-xl font-bold text-gray-900">Data Pengeluaran</h2>
                </div>
                <div className="flex items-center gap-3 sm:gap-4 lg:gap-6 flex-wrap">
                  ...
                  <div className="text-left sm:text-right">
                    <p className="text-[10px] sm:text-xs font-medium text-gray-500 uppercase">Total Pengeluaran</p>
                    <p className="text-base sm:text-lg font-bold text-orange-600 truncate">Rp {totalAmount.toLocaleString('id-ID')}</p>
                  </div>
                </div>
              </div>
